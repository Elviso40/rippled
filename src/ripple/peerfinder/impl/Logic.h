...
public:
    ...
    */
    ...
    void onPeerHandshake (IPAddress const& remote_address, PeerID const& key)
    {
        if (m_journal.debug) m_journal.debug << leftw (18) <<
        ...
        HandshakeAction const action (
            state->slots.onPeerHandshake (peer.inbound(), self, peer.fixed()));
    ...
    void onPeerClosed (IPAddress const& remote_address)
    {
        ...
        case Peer::stateConnected:
            {
                // Update slots
                state->slots.onPeerClosed (peer.inbound(), false, peer.fixed());
                if (peer.outbound())
                {
        ...
        case Peer::stateActive:
            {
                ...
                state->slots.onPeerClosed (peer.inbound(), true, peer.fixed());
                if (m_journal.trace) m_journal.trace << leftw (18) <<
